...
export default function CourseLearningPlayer({
  ...
  // Lesson completion state lives here (not per-lesson local state) so it
  // survives navigating between lessons/modules and can be updated from
  // the server's response after each save.
  const [completedLessonIds, setCompletedLessonIds] = useState<Set<string>>(
    () => new Set(course.modules.flatMap((module) => module.lessons.filter((lesson) => lesson.complete).map((lesson) => lesson.id))),
  )
  ...
  const toggleLessonComplete = async () => {
    if (!lesson || savingCompletion) return

    const nextCompleted = !isLessonComplete
    setSavingCompletion(true)

    try {
      const response = await fetch(`/api/courses/${course.slug}/lessons/${lesson.id}/progress`, {
        method: 'POST',
      ...
      if (!response.ok) {
        console.error(data.error ?? 'Unable to save lesson progress.')
        return
      }
      ...
      console.error('Unable to save lesson progress.')
    } finally {
    ...
    <div className="min-h-[calc(100vh-2rem)] w-full max-w-full overflow-hidden rounded-2xl bg-white shadow-[0_8px_24px_rgba(28,29,82,0.08)]">
      ...
      <div className="grid min-h-[680px] lg:grid-cols-[280px_1fr]">
        <aside className={`${sidebarOpen ? 'block' : 'hidden'} border-b border-slate-200 bg-[#f8fbff] lg:block lg:border-b-0 lg:border-r`}>
          <div className="border-b border-slate-200 p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#5FBB46]">Course outline</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-[#5FBB46]" style={{ width: `${progressPercent}%` }} /></div><p className="mt-2 text-[10px] text-slate-500">{completedLessons} of {totalLessons} lessons complete</p></div>
          <div className="space-y-2 p-3">
            {modules.map((item, moduleIndex) => (
              <div key={item.id} className="rounded-lg">
                ...
                {selectedModule === moduleIndex && <div className="mt-1 space-y-1 pl-3">{item.lessons.length > 0 ? item.lessons.map((itemLesson, lessonIndex) => <button type="button" key={itemLesson.id} onClick={() => selectLesson(lessonIndex)} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-[10px] ${selectedLesson === lessonIndex ? 'bg-blue-50 font-semibold text-blue-700' : 'text-slate-500 hover:bg-white'}`}><span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-white">{completedLessonIds.has(itemLesson.id) ? <Check className="h-3 w-3 text-[#5FBB46]" /> : <Play className="h-2.5 w-2.5" />}</span><span className="truncate">{itemLesson.title}</span></button>) : <div className="px-2.5 py-2 text-[10px] italic text-slate-400">{item.videoUrl || item.content || item.resources.length ? 'Direct module content' : 'No lessons in this module yet'}</div>}</div>}
              </div>
            ))}
          </div>
        </aside>
        ...
        <main className="min-w-0 bg-white">
          <div className="mx-auto max-w-[850px] px-5 py-6 sm:px-8 sm:py-8">
            ...
            {content && <article className="mt-7 max-w-3xl"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#5FBB46]">{lesson ? 'Lesson content' : 'Module content'}</p><div className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-600">{content}</div></article>}

            {resources.length > 0 && <section className="mt-7 rounded-xl border border-slate-200 bg-[#f8fbff] p-4"><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-blue-500" /><h2 className="text-xs font-bold text-[#1C1D52]">Materials for this {lesson ? 'lesson' : 'module'}</h2></div><div className="mt-3 space-y-2">{resources.map((resource) => <a key={resource.id} href={resource.url} target="_blank" rel="noreferrer" className="flex w-full items-center gap-3 rounded-lg bg-white px-3 py-2.5 text-left text-xs text-slate-600 hover:text-blue-600"><Download className="h-3.5 w-3.5 text-[#5FBB46]" />{resource.title}</a>)}</div></section>}
            ...
            {hasLessons && (
              <div className="mt-8 flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-60 ${isLessonComplete ? 'bg-[#e8faf7] text-teal-700' : 'bg-[#5FBB46] text-[#14204f]'}`}
                >
                  {isLessonComplete ? <CheckCircle2 className="h-4 w-4" /> : <Check className="h-4 w-4" />}
                  {savingCompletion ? 'Saving...' : isLessonComplete ? 'Lesson completed' : 'Mark as complete'}
                </button>
                <button type="button" onClick={goNext} disabled={isLastLesson} className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 px-4 py-2.5 text-xs font-semibold text-[#1C1D52] disabled:cursor-not-allowed disabled:opacity-40">Next content<ArrowRight className="h-4 w-4" /></button>
              </div>
            )}
          </div>
        </main>
