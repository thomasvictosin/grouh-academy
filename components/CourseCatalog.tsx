...
// Local alias so the rest of this file can keep saying `Course` without
// re-declaring the shape by hand — the real shape lives in course-data.ts,
// next to serializeCourse(), so it can never drift out of sync again.
...
export default function CourseCatalog() {
  ...
  const categories = ['All', ...new Set(courses.map((course) => course.category?.name).filter((name): name is string => Boolean(name)))]
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses.filter((course) => {
      ...
      const matchesQuery = [course.title, instructorName, categoryName].join(' ').toLowerCase().includes(normalizedQuery)
      const matchesCategory = category === 'All' || categoryName === category
      // NOTE: Course/PublicCourse has no `level` field in the schema.
      // The level dropdown below has therefore never actually filtered
      // anything - it always compared against `undefined`. Neutralized
      // to `true` here rather than faked, pending a decision: either add
      // a `level` column to Course, or remove the level filter UI.
  ...
  return (
    <>
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8 lg:py-14">
        ...
        {error ? <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-6 py-12 text-center text-sm text-red-700">{error}</div> : filteredCourses.length > 0 ? <div className="mt-4 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filteredCourses.map((course) => <article key={course.slug} className="group overflow-hidden rounded-2xl bg-white shadow-[0_8px_24px_rgba(28,29,82,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-xl"><div className="relative aspect-[16/9] overflow-hidden bg-slate-100">{course.thumbnail ? <Image src={course.thumbnail} alt="" fill sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-105" /> : <div className="h-full bg-[linear-gradient(135deg,#1c1d52,#5fbb46)]" />}<span className="absolute left-4 top-4 rounded-full bg-[#141650]/90 px-3 py-1 text-xs font-semibold text-white">{course.category?.name ?? 'Course'}</span></div><div className="p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#4db848]">{course.category?.name ?? 'Academy'}</p><h3 className="mt-2 text-xl font-bold leading-snug">{course.title}</h3><p className="mt-2 text-sm text-slate-500">with {course.creator?.name || 'Grouh Academy'}</p><div className="mt-5 flex items-center gap-3 text-sm text-slate-500"><span className="font-semibold text-[#141650]">{course.rating ? `★ ${course.rating.toFixed(1)}` : 'New course'}</span><span>({course._count?.reviews ?? 0})</span><span className="ml-auto flex items-center gap-1"><BookOpen className="h-4 w-4" />{course.lessonCount} lessons</span></div><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-2xl font-black">{course.currency} {course.price}</span><Link href={`/student/my-courses/${course.slug}/preview`} className="inline-flex items-center gap-2 rounded-lg bg-[#5fbb46] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#4aaa3e]">View course <ArrowRight className="h-4 w-4" /></Link></div></div></article>)}</div> : !loading && <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><h3 className="text-lg font-bold">No published courses match those filters.</h3><p className="mt-2 text-sm text-slate-500">Try a different search term or reset one of the filters.</p></div>}
      </section>
