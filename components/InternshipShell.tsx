...
export default function InternshipShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-dvh flex-1 flex-col overflow-hidden bg-[#F0F7FF]">
      ...
      <div className="flex min-h-0 flex-1 gap-5 overflow-hidden px-4 pb-4 pt-[72px] sm:px-5 lg:gap-6 lg:px-6">
        ...
        <main className="min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto py-1 sm:py-2 md:ml-[268px] lg:ml-[272px]">{children}</main>
      </div>

      <aside className={`fixed inset-y-4 left-4 z-50 w-64 transform transition-transform duration-300 md:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-[120%]'}`}><InternshipSidebar onNavigate={() => setSidebarOpen(false)} /></aside>
