  ...
  return (
    <>
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8 lg:py-14">
        ...
        {filteredCourses.length > 0 ? <div className="mt-4 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filteredCourses.map((course) => <article key={course.title} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"><div className="relative aspect-[16/9] overflow-hidden bg-slate-100"><Image src={course.image} alt="" fill sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-105" /><span className="absolute left-4 top-4 rounded-full bg-[#141650]/90 px-3 py-1 text-xs font-semibold text-white">{course.level}</span></div><div className="p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#4db848]">{course.category}</p><h3 className="mt-2 text-xl font-bold leading-snug">{course.title}</h3><p className="mt-2 text-sm text-slate-500">with {course.instructor}</p><div className="mt-5 flex items-center gap-3 text-sm text-slate-500"><span className="font-semibold text-[#141650]">★ {course.rating}</span><span>({course.reviews})</span><span className="ml-auto flex items-center gap-1"><Clock3 className="h-4 w-4" />{course.duration}</span></div><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-2xl font-black">${course.price}</span><button type="button" className="inline-flex items-center gap-2 rounded-full bg-[#5fbb46] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#4aaa3e]">View course <ArrowRight className="h-4 w-4" /></button></div></div></article>)}</div> : <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><h3 className="text-lg font-bold">No courses match those filters.</h3><p className="mt-2 text-sm text-slate-500">Try a different search term or reset one of the filters.</p></div>}
      </section>
