...
function buildDateBuckets(
  ...
  if (period === '30 Days') {
    return Array.from({ length: 4 }, (_, index) => {
      ...
      const count = Array.from(countsByDate.entries()).reduce(
        (sum, [key, value]) => {
  ...
  return labels.map((label, index) => {
    const monthStart = new Date(
      today.getFullYear(),
      today.getMonth() - (labels.length - 1 - index),
      1,
    )

    const monthEnd = new Date(
      today.getFullYear(),
      today.getMonth() - (labels.length - 2 - index) + 1,
      0,
      23,
      59,
      59,
...
function buildEnrollmentTrendData(
  enrollments: EnrollmentRecord[],
): Record<Period, EnrollmentPoint[]> {
...
export async function GET() {
  ...
  console.log(
    `[admin/student/dashboard] Prisma batch: ${Math.round(
  ...
  const courseRevenue = paidPayments.reduce(
    (sum, payment) =>
      sum + (payment.courseId ? payment.amount : 0),
    0,
  )

  const programRevenue = paidPayments.reduce(
    (sum, payment) =>
      sum + (payment.programId ? payment.amount : 0),
    0,
  )
  ...
  const performanceCourses: DashboardCourse[] =
    courses.map((course) => {
      ...
      return {
        ...
        instructor:
          instructorNames || 'Unassigned instructor',
  ...
  const performanceCoursesSorted =
    [...performanceCourses].sort((a, b) => {
      const aTotal = Object.values(a.periods).reduce(
        (sum, count) => sum + count,
        0,
      )

      const bTotal = Object.values(b.periods).reduce(
        (sum, count) => sum + count,
        0,
      )
  ...
  const certificateRevenue =
    certificateRevenueEstimate(certificates.length)
      ...
      share:
        totalRevenue > 0
          ? Math.max(
              10,
              Math.round(
                (programRevenue / totalRevenue) * 100,
              ),
            )
          : 0,
      color: 'bg-[#5FBB46]',
  ...
  const courseSales = paidPayments.reduce(
    (sum, payment) =>
      sum + (payment.courseId ? 1 : 0),
    0,
  )
  ...
  const stats: DashboardStats[] = [
    ...
    {
      value: formatCompactNumber(courseSales),
      label: 'Course Sales',
      change: '18.5%',
      tone: 'blue',
    },
    ...
    revenueStats: [
...

      {
        ...
        detail: `${formatCompactNumber(
          failedOrCancelledPayments.length,
        )} refunded or failed transactions`,
        tone: 'text-[#1C1D52]',
      },

      {
        ...
        detail: `Due in ${Math.max(
          1,
          Math.min(
            7,
            pendingPayouts > 0 ? 5 : 1,
          ),
        )} days`,
        tone: 'text-[#1C1D52]',
      },
