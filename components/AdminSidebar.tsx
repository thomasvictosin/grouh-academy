...
export default function AdminSidebar({ workspace, admin, mobile = false }: { workspace: 'student' | 'internship'; admin: AdminContext | null; mobile?: boolean }) {
  ...
  return <aside className={`flex-col overflow-hidden rounded-[28px] bg-[#1C1D52] px-4 py-6 text-white shadow-[0_12px_40px_rgba(28,29,82,0.18)] sm:py-8 ${mobile ? 'fixed inset-y-4 left-4 z-40 flex h-auto w-[calc(100vw-2rem)] max-w-64 md:hidden' : 'hidden h-full w-[248px] shrink-0 md:flex'}`}><div className="border-b border-white/10 px-2 pb-5 text-center"><div className="mb-3 flex justify-center"><Image src={admin?.avatarUrl ?? '/avatar-placeholder.png'} alt={`${admin?.name ?? 'Administrator'} avatar`} width={78} height={78} className="h-[78px] w-[78px] rounded-full object-cover" /></div><h3 className="text-sm font-semibold tracking-tight">{admin?.name ?? 'Administrator'}</h3><p className="mt-1 text-xs font-medium text-white/55">Administrator</p></div><nav aria-label="Admin navigation" className="mt-7 flex-1 overflow-y-auto px-1"><ul className="space-y-1">{items.map((item) => { const Icon = item.icon; const active = item.href === `/admin/${workspace}` ? pathname === item.href : pathname.startsWith(item.href); return <li key={item.label}><Link href={item.href} className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition ${active ? 'bg-[#5FBB46] text-white' : 'text-white/90 hover:bg-white/10'}`}><Icon className="h-5 w-5 shrink-0" strokeWidth={1.75} />{item.label}</Link></li> })}</ul></nav><Link href="/admin/logout" className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-white/90 hover:bg-white/10"><LogOut className="h-5 w-5 shrink-0" strokeWidth={1.75} />Log Out</Link></aside>
}
